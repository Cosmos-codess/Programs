...
function runAnalysis(c){showView('analysis');$('analysisContent').innerHTML='<div class="eyebrow">ANALYSIS</div><h2>Running quality and 9-point checks…</h2><p class="lead">The image is being measured before a classification is shown.</p>';setTimeout(()=>{const ctx=c.getContext('2d',{willReadFrequently:true});const W=c.width,H=c.height;const ref=ninePoint(ctx,{x:.08,y:.72,w:.34,h:.20,sw:.035,sh:.035},W,H);const test=ninePoint(ctx,{x:.22,y:.14,w:.56,h:.28,sw:.035,sh:.035},W,H);const whole=sampleRegion(ctx,0,0,1,1,W,H);const quality={brightness:whole.avgLum,glare:glare(ctx),sharpness:sharpness(c),brightnessPass:whole.avgLum>=45&&whole.avgLum<=215,glarePass:whole.glare<8,refPass:ref.valid,testPass:test.valid};const reagent=$('testType').value;const classification=classify(ref,test,reagent);const qualityPassed=quality.brightnessPass&&quality.glarePass&&quality.sharpness>=35&&ref.valid&&test.valid;pendingRecord={analysis:{ref,test,quality,classification,reagent}};renderAnalysis(pendingRecord.analysis,qualityPassed)},350)}
